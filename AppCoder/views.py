...
def cursos(request):
    ...
    return render(request, "AppCoder/cursos.html", {"cursos":cursos, "avatar":obtenerAvatar(request)})
...
def estudiantes(request):
    ...
    return render(request, "AppCoder/estudiantes.html", {"estudiantes":estudiantes,"avatar":obtenerAvatar(request)})

def entregables(request):

    entregables = Entregable.objects.all()

    return render(request, "AppCoder/entregables.html", {"entregables":entregables,"avatar":obtenerAvatar(request)})

def inicioApp(request):

    listado_cursos = Curso.objects.all()

    return render(request, "AppCoder/inicio.html", {"listado_cursos": listado_cursos, "avatar":obtenerAvatar(request)})
...
class EstudianteCreacion(LoginRequiredMixin, CreateView):#vista usada para CREAR
    model= Estudiante
    template_name="Appcoder/estudiante-crear.html"
...
class EstudianteDelete(LoginRequiredMixin, DeleteView):#vista usada para ELIMINAR
    model=Estudiante
    success_url= reverse_lazy("estudiante_list")
    template_name= "AppCoder/estudiante-borrar.html"
...
class CursoUpdate(LoginRequiredMixin, UpdateView):
    model = Curso
    template_name= "AppCoder/curso-editar.html"
    success_url = reverse_lazy('cursos')
    fields= "__all__"

class ProfesorDetalle(LoginRequiredMixin, DetailView):
    model = Profesor
    template_name= "AppCoder/profesor-detalle.html"
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            if usuario is not None:
                ...
                # return render(request, "AppCoder/inicio.html", {"mensaje":f"{usu} has iniciado sesión correctamente", "avatar":obtenerAvatar(request)})
            else:
                return render(request, "AppCoder/login.html", {"form": form, "mensaje":"Usuario o contraseña incorrectos"})
        else:
            return render(request, "AppCoder/login.html", {"form": form, "mensaje":"Usuario o contraseña incorrectos", "avatar":obtenerAvatar(request)})
    else:
...
def editarPerfil(request):
    ...
    if request.method=="POST":
        ...
        return render(request, "AppCoder/editarPerfil.html", {"avatar":obtenerAvatar(request),"form": form, "nombreusuario":usuario.username})
